...
const json = (b: unknown, s = 200, maxAge = 600) =>
  new Response(JSON.stringify(b), {
    status: s,
    headers: { ...CORS, "Content-Type": "application/json", "Cache-Control": `public, max-age=${maxAge}` },
  });

/** "20.5N" -> 20.5 ; "167.2W" -> -167.2 */
function coord(raw: string): number | null {
...
/** Quadrant radii (nautical miles) -> a closed polygon ring. */
function radiiPolygon(
  lat: number, lon: number,
  q: { ne: number; se: number; sw: number; nw: number },
  kt: number, color: string,
) {
  ...
  for (let deg = 0; deg <= 360; deg += 5) {
    ...
    if (r <= 0) { // no extent in this quadrant — collapse to the centre
      ring.push([lon, lat]);
      continue;
    }
  ...
  return {
    ...
    properties: { wind_kt: kt, __color: color, label: `${kt} kt` },
...
Deno.serve(async (req: Request) => {
  ...
  if (!want) return json({ error: "pass ?storm=cp012026" }, 400, 60);

  try {
    const cs = await fetch("https://www.nhc.noaa.gov/CurrentStorms.json", {
      headers: { "User-Agent": UA, Accept: "application/json" },
    });
    if (!cs.ok) return json({ radii: { type: "FeatureCollection", features: [] }, stormId: want }, 200, 120);
    const data = await cs.json() as { activeStorms?: Record<string, unknown>[] };
    const storm = (data.activeStorms ?? []).find((s) => String(s.id ?? "").toLowerCase() === want);
    if (!storm) return json({ radii: { type: "FeatureCollection", features: [] }, stormId: want, reason: "storm not active" }, 200, 300);

    const advUrl = (storm.forecastAdvisory as { url?: string } | undefined)?.url;
    if (!advUrl) return json({ radii: { type: "FeatureCollection", features: [] }, stormId: want, reason: "no forecast advisory" }, 200, 300);

    const adv = await fetch(advUrl, { headers: { "User-Agent": UA } });
    if (!adv.ok) return json({ radii: { type: "FeatureCollection", features: [] }, stormId: want }, 200, 120);
    ...
    if (lat === null || lon === null) {
      return json({ radii: { type: "FeatureCollection", features: [] }, stormId: want, reason: "no centre fix" }, 200, 300);
    }
