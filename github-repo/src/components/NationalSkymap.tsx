...
// Purple-focused palette — brighter, no blue/cyan
...
interface Props { nightHour?: number }
...
export function NationalSkymap({ nightHour = 22 }: Props) {
      ...
      try {
        ...
        L.geoJSON(geoData, {
          style: (feature) => {
            ...
            return { fillColor: bucket.fill, fillOpacity: 0.72, color: bucket.stroke, weight: 0.8, opacity: 0.7 };
          },
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden border border-purple-800/40 bg-[#07070F]"
      style={{ boxShadow: "0 0 40px rgba(192,132,252,0.10)" }}>
      ...
      <div className="absolute bottom-3 right-3 bg-black/80 rounded-lg px-3 py-2 space-y-1 pointer-events-none">
        <div className="text-[9px] uppercase tracking-[0.12em] text-white/50 font-semibold mb-1">Sky Conditions</div>
        {LEGEND.map(l => (
      ...
      {status === "error" && (
        <div className="absolute inset-0 flex items-center justify-center text-sm text-muted-foreground pointer-events-none">
          Couldn't load the sky map. Try refreshing.
        </div>
      )}
    </div>
  );
