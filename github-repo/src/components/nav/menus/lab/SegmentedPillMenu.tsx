 ...
 * I1 · Segmented Pill.
 *
 * A floating pill above the content rather than docked to the edge, with a
 * thumb that slides under whichever segment you are on. The page keeps its full
 * height behind it.
 *
 * THE THUMB IS MEASURED. Segment widths come from their text, so they are all
 * different and no arithmetic on index gives the right offset. The thumb reads
 * the focused button's own box, which also means it re-measures for free when
 * the label set changes — drilling into a section swaps six section names for
 * however many module names, and the thumb simply lands on the new first one.
 *
 * The pill scrolls horizontally when the labels do not fit, which on a phone is
 * most of the time. That is why the thumb is positioned against the scrolling
 * track rather than the pill's outer box: it has to travel with the content.
 */
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 z-[60] pointer-events-none">
      ...
      <AnimatePresence>
        {open && (
          <motion.nav
            className="absolute left-1/2 flex items-center gap-1 overflow-hidden"
            style={{
          ...
          >
            ...
            <div ref={trackRef} className="relative flex items-center gap-0.5 overflow-x-auto"
              ...
              />
              {entries.map((e, i) => (
                <div key={e.key} ref={(el) => { segRefs.current[i] = el; }} className="relative flex-none">
                  <EntryAction
                    entry={e} nav={nav}
                    className="relative rounded-full whitespace-nowrap block"
                    style={{
                      padding: "8px 13px", fontSize: 11.5, fontWeight: 700, fontFamily: HEADING,
                      color: focus === i ? ROYAL.text : ROYAL.dim,
                      transition: calm ? "none" : "color .24s",
                    }}
                  >
                    <span onPointerEnter={() => setFocus(i)} onFocus={() => setFocus(i)}>{e.label}</span>
                  </EntryAction>
                </div>
              ))}
            </div>
